...
tic()
usa_00002 = readRDS("D:/Economics/Data/Census/usa_00002.rds") %>%
  filter( YEAR == 1980 ) %>%
  select( YEAR , 
          STATEFIP , 
          COUNTYFIP , 
          ...
          EDUC , 
          MARST ) %>%
  mutate( ever_married = ifelse( MARST != 6 ,
                                 PERWT ,
                                 0 ) 
  )   %>%
  select( -MARST) %>%
...
county_1980_census = usa_00002 %>% 
  ...
  mutate( countyfips = STATEFIP*1000 + COUNTYFIP ) %>%
  select( - COUNTYFIP )
toc()
...
usa_00002 = readRDS("D:/Economics/Data/Census/usa_00002.rds") %>%
  ...
  select( YEAR , 
          ...
          INCTOT , 
          INCWAGE )  %>%
  mutate( ilf = ifelse(LABFORCE==2 , PERWT , 0 ) ,
          unemployed = ifelse(EMPSTAT==2 , PERWT , 0 ) ,
          black = ifelse( RACE==2 , PERWT , 0 ) ,) %>%
  mutate_all( .funs = as.numeric)

county_1980_census_laborforce = usa_00002 %>%
  group_by( YEAR , 
            STATEFIP , 
            COUNTYFIP ) %>%
  summarise( ilf = sum(ilf) ,
             unemployed = sum(unemployed) , 
             black = sum(black) ,
             income = weighted.mean( INCTOT , 
                                     w = PERWT ) ,
             wages = weighted.mean( INCWAGE , 
                                    w = PERWT ) , 
             sample_lf = n()
  ) %>%
  mutate( countyfips = STATEFIP*1000 + COUNTYFIP ) %>%
...
us_census_1980 = merge( county_1980_census ,
                        county_1980_census_laborforce ) %>%
  mutate( lfpr = ilf/pop ,
          uer = unemployed/ilf , 
          black_rate = black/pop ,
          coupling_rate = ever_married/pop )

rm(usa_00002 ,
   county_1980_census ,
   county_1980_census_laborforce )

saveRDS( us_census_1980 , 
         "us_census_1980.RDS" )
...
county_1990_census_laborforce = usa_00002 %>%
  group_by( YEAR , 
            STATEFIP , 
            COUNTYFIP ) %>%
  summarise( ilf = sum(ilf) ,
...
us_census_1990 = merge( county_1990_census ,
                        county_1990_census_laborforce ) %>%
  mutate( lfpr = ilf/pop ,
          uer = unemployed/ilf , 
          black_rate = black/pop ,
          coupling_rate = ever_married/pop )
...
county_2000_census_laborforce = usa_00002 %>%
  group_by( YEAR , 
            STATEFIP , 
            COUNTYFIP ) %>%
  summarise( ilf = sum(ilf) ,
...
us_census_2000 = merge( county_2000_census ,
                        county_2000_census_laborforce ) %>%
  mutate( lfpr = ilf/pop ,
          uer = unemployed/ilf , 
          black_rate = black/pop ,
          coupling_rate = ever_married/pop )
...
census_2010 = read_ipums_micro( ddi ) %>% 
  filter( YEAR == 2010 ) %>%
  mutate( ilf = ifelse(LABFORCE==2 , PERWT , 0 ) ,
          unemployed = ifelse(EMPSTAT==2 , PERWT , 0 ) ,
          student = ifelse( EDUC < 7 & AGE < 19 , PERWT , 0 ) ,
          women = ifelse(SEX==2 , PERWT , 0 ) , 
          black = ifelse( RACE==2 , PERWT , 0 )) %>%
  lapply( as.numeric) %>% 
  bind_cols()

census_2010$EDUC = census_2010$EDUC %>%
  recode('0' = 0 ,
         '1' = 2.5 ,
         '2' = 6.5 ,
         '3' = 9 ,
...
tic()
county_2010_census = census_2010 %>% 
  group_by( YEAR , 
            STATEFIP , 
            COUNTYFIP ) %>%
...
county_2010_census_laborforce = census_2010 %>%
  group_by( YEAR , 
            STATEFIP , 
            COUNTYFIP ) %>%
  summarise( ilf = sum(ilf) ,
...
us_census_2010 = merge( county_2010_census ,
                        county_2010_census_laborforce ) %>%
  mutate( lfpr = ilf/population ,
          uer = unemployed/ilf , 
          black_rate = black/population )

rm(county_2010_census ,
   county_2010_census_laborforce )

saveRDS( us_census_2010 , 
         "us_census_2010.RDS" )

us_census_1980 = readRDS("D:/Economics/Projects/macro-development/us_census_1980.RDS") %>% 
  select( -c(sample, sample_lf , ever_married, coupling_rate)) %>%
  rename( population = pop )
us_census_1990 = readRDS("D:/Economics/Projects/macro-development/us_census_1990.RDS") %>% 
  select( -c(sample, sample_lf , ever_married, coupling_rate)) %>%
  rename( population = pop )
us_census_2000 = readRDS("D:/Economics/Projects/macro-development/us_census_2000.RDS") %>% 
  select( -c(sample, sample_lf , ever_married, coupling_rate)) %>%
  rename( population = pop )
us_census_2010 = readRDS("D:/Economics/Projects/macro-development/us_census_2010.RDS")
